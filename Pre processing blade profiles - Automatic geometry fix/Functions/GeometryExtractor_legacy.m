...
function Report=GeometryExtractor_legacy(stlPath,AirfDir,GraphDir,PADDir,GenerateGif,steps,d,Holdgraphs,DebugGraphNumbered,range_rpm,UnitFactor,Soundspeed,ni)
    ...
    for z = zscan
        ...
        for c=1:size(XX,1)                                                      %Counter for not losing points ids
    
            if (XX(c,3)<=z+d_used) && (XX(c,3)>=z-d_used)                       %check if point is in z+-d interval
                ...
                parfor a=1:size(Connections,1)                                     %scan whole connections matrix
